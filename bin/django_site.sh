...
rootdir=$(cd ..;pwd)
pidfile=$curdir/django.pid
logfile=$curdir/django_access.log
SITEPATH=$rootdir
PYTHON=python3
CMD="$SITEPATH/manage.py"
app_name='saltmgt'
...
then
    ...
    nohup $PYTHON "$CMD" runserver 0:8080 >$logfile 2>&1 &
    
    if [ $? -eq 0 ];then

        echo "$!" >$pidfile
        echo "django site $app_name is up"
    fi
fi
...
stop () {
    ...
    then
        ...
        pkill -9 -g $gid && { rm $pidfile; return 0; }
    else
...
case "$1" in
    ...
    restart)
        ...
        ;;
    stop)
        stop && echo "django site $app_name stopped"
        ;;
    status)
        status
        ;;
